...
const ProductManager = () => {
  ...
  const handleProductViewsChange = (e) => {
    const files = Array.from(e.target.files);
    setFormData(prev => ({ ...prev, productViews: files }));

    const previews = files.slice(0, 4).map(file => URL.createObjectURL(file));
    setPreviewProductViews(previews);
  };

  const handleSingleProductViewChange = (e, index) => {
    const file = e.target.files[0];
    if (!file) return;

    const newFiles = [...formData.productViews];
    newFiles[index] = file;
    setFormData(prev => ({ ...prev, productViews: newFiles }));
        ...
        <Form onSubmit={handleSubmit} encType="multipart/form-data">
          <Row>
            ...
            <Col md={6}>
              ...
              <Form.Group className="mb-2">
                <Form.Label>Product Views (Multiple Images)</Form.Label>
                <Form.Control type="file" name="productViews" accept="image/*" multiple onChange={handleProductViewsChange} />
                <div className="d-flex gap-2 mt-2">
                  {[0, 1, 2, 3].map(index => (
                    <div key={index} style={{ position: 'relative', width: '80px', height: '80px', border: '2px dashed #ddd', borderRadius: '8px', overflow: 'hidden', background: '#f8f9fa', cursor: 'pointer' }}
                      onClick={() => document.getElementById(`productView${index}`).click()}>
                      {previewProductViews[index] ? (
                        <img src={previewProductViews[index]} alt={`View ${index + 1}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                      ) : (
                        <div className="d-flex align-items-center justify-content-center h-100 text-muted" style={{ fontSize: '11px' }}>+ Add</div>
                      )}
                      <input type="file" id={`productView${index}`} accept="image/*" style={{ display: 'none' }} onChange={(e) => handleSingleProductViewChange(e, index)} />
                    </div>
                  ))}
                </div>
              </Form.Group>
